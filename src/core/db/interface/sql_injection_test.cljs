#_(ns core.db.interface.sql-injection-test
    (:require
     [clojure.core.async :refer [<!] :refer-macros [go]]
     [cljs.test :refer-macros [deftest testing is async]]
     [core.db.impl]
     [core.db.interface :as db-conn]
     [core.db.interface.fixture :refer [create-table-query insert-query select-query]]))

#_(deftest sql-injection-test
    (async done
           (testing "Prevents SQL injection attacks"
             (go
               (let [db-conn (db-conn/new! {:db/impl :db/impl-better-sqlite3})
